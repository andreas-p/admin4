...
# Constant to define standard window name
STD_NAME = '_XRCED_T_W'
...
class PullDownMenu:
    ...
    def __init__(self, parent):
        ...
        self.createMap.update(customCreateMap)
...
#             (ID_NEW.WIZARD_PAGE, 'Wizard Page', 'Create wizard page'),
        ...
        if customControls:
          self.controls.append(customControls)
        ...
        if customControls:
          self.toolBarControls.append(customControls)
        self.stdButtons = [
...
#                return
        ...
        res = xrc.XmlResource('memory:xxx.xrc', xmlFlags)
        if reshandler:
            res.AddHandler(reshandler())
        try:
            if xxx.__class__ == xxxFrame:
                # Frame can't have many children,
                # but it's first child possibly can...
...
try:
  loaddir="%s/.." % os.path.dirname(os.path.abspath(__file__))
  f, pathname, description=imp.find_module("xmlres", [loaddir])
  mod=imp.load_module("xmlres", f, pathname, description)
  mod.init(loaddir)
  reshandler=mod.XmlResourceHandler
except:
  reshandler=None  

if reshandler:
  customControls = ['custom', 'Custom controls']
  for ctl in mod.xmlControlList:
    xxxDict[ctl] = xxxUnknown
    id=wx.NewId()
    customCreateMap[id] = ctl
    customControls.append( (id, ctl, "Create %s Control" %ctl) )
  ID_NEW.LAST =wx.NewId() 
